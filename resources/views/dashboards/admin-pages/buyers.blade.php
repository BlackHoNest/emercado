@php
   use Illuminate\Support\Str;
    use App\Http\Controllers\AESCipher;
    $aes = new AESCipher();
@endphp

<x-app-layout :assets="$assets ?? []">
   <div class="row">
      <div class="col-md-12 col-lg-12">
         <div class="row row-cols-1">
            <div class="d-slider1 overflow-hidden ">
                <div class="overflow-auto">
                    <table class="table bg-white rounded table-hover">
                        <thead>
                            <tr class="table-info">
                            <th scope="col">#</th>
                            <th scope="col">Buyer Name</th>
                            <th scope="col">Province</th>
                            <th scope="col">Municipality</th>
                            <th scope="col">Barangay</th>
                            <th scope="col">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php $count = 1;  @endphp
                            @foreach ($buyers as $buyer)
                                <tr class='{{ $buyer->buyer_id }}'>
                                    <td data-target='accountid' hidden>{{ $aes->encrypt($buyer->buyer_id) }}</td>
                                    <th scope="row">{{ $count }}</th>
                                    <td data-target='buyer_name'><img src="{{asset('storage/images/client/photo/'. $buyer->profile_picture)}}" alt="User-Profile" class=" theme-color-default-img img-fluid avatar avatar-50 avatar-rounded">
                                        <span class='ms-3'>{{ ucwords($buyer->last_name) }}, {{ ucwords($buyer->first_name) }}</span>
                                    </td>
                                    <td>{{ ucwords($buyer->provDesc) }}</td>
                                    <td>{{ ucwords($buyer->citymunDesc) }}</td>
                                    <td>{{ ucwords($buyer->brgyDesc) }}</td>
                                    <td>
                                    <form class="d-inline" method="GET" action="{{ route('view.buyer') }}" data-toggle="validator" enctype="multipart/form-data">
                                        @csrf
                                            <input type="text" class="form-control" name='buyer_id' value='{{ $aes->encrypt($buyer->buyer_id) }}' hidden>
                                            <button class="btn btn-success" title="View" type="submit">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-fill" viewBox="0 0 16 16">
                                                <path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z"/>
                                                <path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/>
                                            </svg>
                                            </button>
                                        </form>
                                        <button class="btn btn-danger" data-id='{{ $buyer->buyer_id }}' data-role='delete_buyer' title="Delete">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3-fill" viewBox="0 0 16 16">
                                                <path d="M11 1.5v1h3.5a.5.5 0 0 1 0 1h-.538l-.853 10.66A2 2 0 0 1 11.115 16h-6.23a2 2 0 0 1-1.994-1.84L2.038 3.5H1.5a.5.5 0 0 1 0-1H5v-1A1.5 1.5 0 0 1 6.5 0h3A1.5 1.5 0 0 1 11 1.5Zm-5 0v1h4v-1a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5ZM4.5 5.029l.5 8.5a.5.5 0 1 0 .998-.06l-.5-8.5a.5.5 0 1 0-.998.06Zm6.53-.528a.5.5 0 0 0-.528.47l-.5 8.5a.5.5 0 0 0 .998.058l.5-8.5a.5.5 0 0 0-.47-.528ZM8 4.5a.5.5 0 0 0-.5.5v8.5a.5.5 0 0 0 1 0V5a.5.5 0 0 0-.5-.5Z"/>
                                            </svg>
                                        </button>
                                    </td>
                                </tr>
                                @php $count += 1;  @endphp
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
         </div>
      </div>
   </div>
</x-app-layout>
@extends('modals.delete-info')